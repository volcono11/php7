     ...
     if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
     {
           ...
           if(s1.toString() == s2.toString()){
             alertify.alert("Record Saved", function () {
              document.frmChildEdit.action='object_dbfileds.php?PARENTID='+document.getElementById('txt_A_objectid').value;
...
function updateChildrecord(childid){
    ...
    document.frmChildEdit.action='object_dbfileds.php?CHILDID='+childid+'&PARENTID='+document.getElementById('txt_A_objectid').value;
    document.frmChildEdit.submit();
}
...
function deleteChildrecord(childid){
        ...
        alertify.confirm("Are you sure you want to delete ? ", function (e) {
         if (e) {
           document.frmChildEdit.action='object_dbfileds.php?DEL=DELETE&CHILDID='+childid+'&PARENTID='+document.getElementById('txt_A_objectid').value;
           document.frmChildEdit.submit();
         } else {
            return;
         }
...
$frmPage_startrow = isset($_REQUEST['frmPage_startrow']) ? $_REQUEST['frmPage_startrow'] : '';
    ...
    $formlistname = "object_dbfileds.php";

    $grid = new MyPHPGrid('frmPage');

    $grid->formName = "object_dbfileds.php";

    $grid->inpage = $frmPage_startrow;

    $grid->TableNameChild = "tbl_objectdbfiled";
...
if($CHILDID !='' && $DEL =='DELETE'){
        mysqli_query($con,"delete from tbl_objectdbfiled where id='". $CHILDID."'");
        $CHILDID ="";

}
                                  
/*        $sql_1 = "select * from tbl_objectdbfiled where id='".$CHILDID."'";
    ...
    if($CHILDID !='' && $DEL !='DELETE'){
		$display="table-row";
	       $SEL12 = "Select * from tbl_objectdbfiled where id ='".$CHILDID."'";
        ...
        $no_of_rows = mysqli_num_rows(mysqli_query($con,"select * from tbl_objectdbfiled where objectid='".$PARENTID."'"));
        ...
        $entrydata = "<div class='table-responsive no-padding'>
            <form name='frmChildEdit' method='post' id='frmChildEdit' autocomplete='off' enctype='multipart/form-data'>
                <table class='table table-bordered table-condensed table-fixed  table-responsive' style='table-layout:fixed'>
                    <tr>
                        <td class='dvtCellLabel' style='border: 1px solid #ccc;width:13%;'>Filedname $mandatory</td>
                        ...
                        <td style='border: 1px solid #fff;' colspan=2>&nbsp;</td>
                    </tr>
                </table>
            </form>
...
if(isset($_REQUEST['search'])!=""){
	$addsql = " and (";
	$addsql .= " f ieldname like '%".$_REQUEST['search']."%'";
	$addsql .= ")";
}

$list_sql = "SELECT tbl_objectdbfiled.* FROM tbl_objectdbfiled where objectid='".$PARENTID."' $addsql";
...
$result = mysqli_query($con,$sql) or die(mysqli_error());
        ...
        $entrydatatable.= "<th class='bg-light-blue' style='width:15%;text-align:left;border-bottom:1px #2F3C43 solid;border-top:1px #fff solid;color:#FFFFFF'>Field</th>";
		...
		while($loginResultArrayChild   = mysqli_fetch_array($result)){
        	...
        	$entrydatatable.= "<tr>";
        	$entrydatatable.= "<td style='background-color:$colorbg;color:$colorfc;border:1px #ccc solid;'>" . $i++. "</td>";
        	$entrydatatable.= "<td style='background-color:$colorbg;color:$colorfc;border:1px #ccc solid;'>" . $loginResultArrayChild['fieldname']. "</td>";
        	if($update == "true")
