...
<h2 class='title'>USERS</h2>
    	...
    	$insert = $update = $delete = "false";
          ...
          $id_field = 'in_user.id';
          $fieldNames = array('','User ID','User Name','Email ID','Role','Password');
          $fieldSizes = array('',10,16,20,35,15);
          $fieldAlign = array('','left','left','left','left','left');
          $fields = array('in_user.userid','in_user.username','in_user.email', 'in_user.rolecode', 'in_user.pwd');
          $table = 'in_user';
          $formlistname = "newuserdetails.php";
          $formeditlist= "editnewuserdetails.php";
          ...
          $clause = "(in_user.username LIKE '%$findBysearch%' or in_user.userid LIKE '%$findBysearch%')
                     and (in_user.rolecode like '%1001%' or in_user.rolecode like '%1002%')";
          }else{
          $clause = "(in_user.username LIKE '%$findBysearch%' or in_user.userid LIKE '%$findBysearch%')
                     and (in_user.rolecode like '%1001%' or in_user.rolecode like '%1002%') ";
          }
          ...
          $grid->editFields = array("userid,username,pwd,email,companycode");

          $grid->editRecord = $update;

          $grid->TabsNames = array("User Details:1:6");

          $grid->TableName = "in_user";

          $grid->formName = "newuserdetails.php";
