...
$grid->formName = "mainmenulist.php";
...
if(isset($_REQUEST['dr'])=='view'){
   $edit="none";
   $view="inline";
   $title="Viewing Main Menu : $name";
}else if(isset($_REQUEST['dr'])=='edit'){
      $edit="inline";
      $view="none";
      $title="Editing Main Menu : $name";
}else{
      $edit="inline";
      $view="none";
       ...
       insertfunction(parameter,action)
                         ...
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                // alertify.alert("Record Saved", function () {
                                 window.location.href='editmainmenulist.php?dr=edit&ID='+document.getElementById('saveid').value;
                               // });
                               }else if(s1.toString() == s3.toString()){
                               // alertify.alert("Record Updated", function () {
                                window.location.href='editmainmenulist.php?dr=edit&ID='+document.getElementById('mode').value;
                    ...
                    function stateChangedsavenew()
                   {
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                 alertify.alert("Record Saved", function () {
                                 window.location.href='editmainmenulist.php?dr=add&ID=0';
                                });
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='editmainmenulist.php?dr=add&ID=0';
                    ...
                    function stateChangedsaveclose()
                   {
                         ...
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                ...
                                window.location.href='mainmenulist.php?ID=0';
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='mainmenulist.php';
...
<body class="hold-transition sidebar-mini">
    ...
    <section class="content-header">
                 ...
                 <a class="pull-left" href="mainmenulist.php" data-toggle="tooltip" data-placement="right" title="Back to Menu Setup"><i class='fa fa-backward'></i></a>
                 <h2 class="title">&nbsp;&nbsp;<?echo $title; ?></h2>

                 <ol class='breadcrumb'>
                  <li><a href="#"><a href="blank.php" >Menu Setup</a></li>
                  <li><a href="#"><a href="mainmenulist.php?ps=1">Main Menu</a></li>
...
<section class="content" id='content-content-id' style='padding-right:5px;padding-left:5px;margin-top:-10px;' >
                  ...
                  <div class="nav-tabs-custom" id="nav-tabs-custom-id">
                        <ul class="nav nav-tabs">
                           <li class="active"><a href="#personal" data-toggle="tab"><i class="fa fa-desktop" aria-hidden="true"></i>&nbsp; Main Menu</a></li>
                        </ul>
       ...
       $entrydata  = " <form name='frmEdit' method='post' id='frmEdit' enctype='multipart/form-data'>
                             <table class='table table-bordered table-condensed table-fixed  table-responsive' style='table-layout:fixed'>
                                                            <tr>
                                                              ...
                                                              <td style='border: 1px solid #ccc;'><input type='text' class='form-control txt inputs' name='txt_A_name' id='txt_A_name'  value='$name' ></td>
                                                              <td class='dvtCellLabel' style='border: 1px solid #ccc;'>Icon Image:</td>
                                                              <td style='border: 1px solid #ccc;'><input type='text' class='form-control txt inputs' name='txt_A_iconimage' id='txt_A_iconimage'  value='$iconimage' ></td>
                                                            </tr>
                                                            <tr>
                                                              <td class='dvtCellLabel' style='border: 1px solid #ccc;'>Roles:</td>
                                                              <td colspan=5 style='border: 1px solid #ccc;'>".GetRoles($roles)."
                                        ...
                                        <button class='btn btn-danger inputs' style='margin-top:-5px;' name='btndanger' type='button'  onclick ='javascript:cancleediting(\"mainmenulist.php\");'>Close &nbsp;<i class='fa fa-close' aria-hidden='true'></i></button>
                                        </div>
                                        </form> " ;
...
function GetRoles($e_country) {
	global $con;
         ...
         $mycontrol = "<div class='divcheckbox'>";
                                      ...
                                      $SQLResSub1 =  mysqli_query($con,$SQLsub1) or die(mysql_error()."<br>".$SQLsub1);
                                      ...
                                      if(mysqli_num_rows($SQLResSub1)>=1){
                                        while($loginResultArraySub1   = mysqli_fetch_array($SQLResSub1)){
                                           ...
                                           $mycontrol .= "<input type='checkbox' class='minimal inputs' id='chk_A_roles' name='chk_A_roles[]' value='".$loginResultArraySub1[0]."'".$selected."/>&nbsp;&nbsp;" . $loginResultArraySub1[1]. "&nbsp;&nbsp;";
...
//echo $roles."tyty";
         ...
         $CMB = "<select id='cmb_A_roles' name='cmb_A_roles' class='select2 inputs' multiple='multiple'>";
         $SEL =  "select lookcode,lookname from in_lookup where looktype='ROLES' and lookname<>'XX' order by id";
         $RES = mysql_query($SEL);
