...
function loadfields(catval){
      ...
      var url="combofunctions_setup.php?level=tablefields&table="+catval; 
      xmlHttp.onreadystatechange=stateChangedcombo_3
      xmlHttp.open("POST",url,true)
      xmlHttp.send(null)
}

function stateChangedcombo_3(){
       if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
       {
             var s1 = trim(xmlHttp.responseText);  
             document.getElementById('txt_A_fieldname').value=s1;
       }
}
...
function showData(catval){
	if(catval == "33002")//Field Approval
	{
		...
		document.getElementById('cmb_A_tableid').disabled= false ;
	}
	else{
		document.getElementById('tr1').style.display = 'none';
		document.getElementById('cmb_A_tableid').value= '' ;
		document.getElementById('cmb_A_tableid').disabled= true ;
	}
}
...
if($CHILDID !='' && $DEL =='DELETE'){
		...
		if(mysqli_num_rows($res_1)>=1){
			...
			$canrevise = $arr_1['canrevise'];
			$status = $arr_1['status'];
			$tableid = $arr_1['tableid'];
			$processend = $arr_1['processend'];
			$cancelstatus = $arr_1['cancelstatus'];
			$rejectstatus = $arr_1['rejectstatus'];
			$revisestatus = $arr_1['revisestatus'];
			...
			if($canrevise == "2001") $revise_status_disabled = "";
			else $revise_status_disabled = "disabled";
		}
		else{
			$textonbutton = $fieldname = $approvalmode = $valuefrom = $valueto = $inweb = $firebase = $sms = $email = $noalets = $cancancel = $canreject = $canedit =$status = $tableid = $processend = $canrevise = $cancelstatus = $rejectstatus = $revisestatus ="";
			...
			$revise_status_disabled = "disabled";
		...
		$processstart = ($sequencer == "1") ? 'true' : 'false';
            ...
            <form name='frmChildEdit' method='post' id='frmChildEdit' autocomplete='off' enctype='multipart/form-data'>
                <table class='table table-bordered table-condensed table-fixed  table-responsive' style='table-layout:fixed'>
                    ...
                    <tr>
                        ...
                        <td class='dvtCellLabel' style='border: 1px solid #ccc;width:13%;'>End of the Process $mandatory</td>
                    	<td style='border: 1px solid #ccc;'>".GetTrueorFalse($processend,'cmb_A_processend','')."</td>
                    	<td class='dvtCellLabel' style='border: 1px solid #ccc;width:13%;'>Table Name $mandatory</td>
                    	<td style='border: 1px solid #ccc;'>".GetTable($tableid)."
                    	</td>
                    </tr>
                    ...
                    <tr>
                    	...
                    	<td class='dvtCellLabel' style='border: 1px solid #ccc;width:13%;'>Cancel Status$mandatory</td>                    	                <td style='border: 1px solid #ccc;'>".GetStatus('cmb_A_cancelstatus',$cancelstatus,$cancel_status_disabled)."</td>
                    </tr>
                    <tr>
                    	...
                    	<td style='border: 1px solid #ccc;'>".GetStatus('cmb_A_rejectstatus',$rejectstatus,$reject_status_disabled)."</td> 
                    
                    <tr>
                    	<td class='dvtCellLabel' style='border: 1px solid #ccc;width:13%;'>Can Revise $mandatory</td>
                    	<td style='border: 1px solid #ccc;'>".GetYesNo($canrevise,'cmb_A_canrevise','onchange=javascript:calldisable(this.value,\'cmb_A_revisestatus\')')."</td> 
                    	<td class='dvtCellLabel' style='border: 1px solid #ccc;width:13%;'>Revise Status$mandatory</td>
                    	<td style='border: 1px solid #ccc;'>".GetStatus('cmb_A_revisestatus',$revisestatus,$revise_status_disabled)."</td> 
                    </tr>
                     </tr>	
                    ...
                    <tr>
                        ...
                        <input type='hidden' class=textboxcombo name='txt_A_processstart' id='txt_A_processstart' value='".$processstart."'>
...
$result = mysqli_query($con,$sql) or die(mysqli_error());
		...
		while($loginResultArrayChild   = mysqli_fetch_array($result)){
        	...
        	$colorfc ='#5A5A5A';
        	$start_iocn = $end_icon = '';
        	if($loginResultArrayChild['processstart']=="true") $start_iocn = '<i class="fa fa-star" aria-hidden="true" style="color:green"></i>';
        	if($loginResultArrayChild['processend']=="true") $end_icon = '<i class="fa fa-star" aria-hidden="true" style="color:red"></i>';
        	
        	$entrydatatable.= "<tr id='".$loginResultArrayChild['id']."'>";
        	$entrydatatable.= "<td style='background-color:$colorbg;color:$colorfc;border:1px #ccc solid;'>&nbsp;".
        	$loginResultArrayChild['sequencer']."&nbsp;".$start_iocn.$end_icon."&nbsp;</td>";
...
function GetTable($tableid){
	global $con;
    $CMB = " <select name='cmb_A_tableid'  id='cmb_A_tableid' class='form-control select' onChange='javascript:loadfields(this.value);'>";
    $CMB .= "<option value='' >Select</option>"; 
    $SEL =  "select id,objectname from tbl_objectmaster where objecttype='4003' order by id";
    $RES = mysqli_query($con,$SEL);
	while ($ARR = mysqli_fetch_array($RES)) {
	    $SEL = "";
	    if(strtoupper($tableid) == strtoupper($ARR['id'])){ $SEL =  "SELECTED";}
	    $CMB .= "<option value='".$ARR['id']."' $SEL >".$ARR['objectname']."</option>";
	}
	$CMB .= "</select>";
	return $CMB;
}
function GetTrueorFalse($yesno,$fieldname,$onchange){
	global $con;
    $CMB = " <select name='$fieldname'  id='$fieldname' class='form-control select' $onchange>";
    $CMB .= "<option value='' >Select</option>"; 
    $SEL_yes = ($yesno=="true") ? 'SELECTED' : '';
    $SEL_no =  ($yesno=="false") ? 'SELECTED' : '';
    $CMB .= "<option value='false' $SEL_no >No</option>";    
	$CMB .= "<option value='true' $SEL_yes >Yes</option>";
	$CMB .= "</select>";
	return $CMB;
}
