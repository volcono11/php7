...
//$pagerights = $WF->WorkflowPageRights($_REQUEST['ID'],'tbl_statusimages');
//$Action_button = $WF->actionWorkflow($_REQUEST['ID'],'tbl_statusimages');
...
if($_REQUEST['ID'] != "0") {
             ...
             $SQL = " Select * from tbl_statusimages where id='".$_REQUEST['ID']."'";
             $SQLRes =  mysqli_query($con,$SQL) or die(mysqli_error()."<br>".$SQL);
              if(mysqli_num_rows($SQLRes)>=1){
                while($loginResultArray   = mysqli_fetch_array($SQLRes)){
                   ...
                   $imagename=  $loginResultArray['imagename'];
                  }
              }
           }else{
              $mode="";
              $saveid =  GetLastSqeID("tbl_statusimages");
              $imagename = "";
...
if(isset($_REQUEST['dr'])=='view'){
   $edit="none";
   $view="inline";
   $title="Viewing : ".$imagename."";
}else if(isset($_REQUEST['dr'])=='edit'){
      $edit="inline";
      $view="none";
      $title="Editing : ".$imagename."";
}else{
      $edit="inline";
      $view="none";
...
function editingrecord(action)
{
       ...
       var txt_A_LASTNUMBER=document.getElementById('txt_A_imagename');
       if(txt_A_LASTNUMBER){
          if ((txt_A_LASTNUMBER.value==null)||(txt_A_LASTNUMBER.value=="")){
               alertify.alert("Enter Image Name", function () {
       ...
       insertfunction(get(document.frmEdit),action)
                   ...
                   function stateChangedsave()
                   {
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                 ...
                                 window.location.href='editimagelist.php?dr=edit&ID='+document.getElementById('saveid').value;
                                });
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='editimagelist.php?dr=edit&ID='+document.getElementById('mode').value;
                    ...
                    function stateChangedsavenew()
                   {
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                               alertify.alert("Record Saved", function () {
                                 window.location.href='editimagelist.php?dr=add&ID=0';
                                });
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='editimagelist.php?dr=add&ID=0';
                    ...
                    function stateChangedsaveclose()
                   {
                         ...
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                ...
                                window.location.href='imagelist.php?ID=0';
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='imagelist.php';
...
<body class="hold-transition sidebar-mini">
         ...
         <section class="content-header">
                 ...
                 <a class="pull-left" href="imagelist.php?objectid=<?php echo $_SESSION['objectid']; ?>&pr=<?php echo $_SESSION['pr']; ?>&frmPage_rowcount=<?php echo $_SESSION['frmPage_rowcount']; ?>&txtsearch=<?php echo $_SESSION['txtsearch']; ?>&frmPage_startrow=<?php echo $_SESSION['frmPage_startrow'];?>" data-toggle="tooltip" data-placement="right" title="Back to Home"><i class='fa fa-backward'></i></a>
                 <h2 class="title"><?php echo "&nbsp;&nbsp;".$title; ?></h2>
<!--
                 <ol class='breadcrumb'>
                  <li><a href="#"><a href="blank.php" >Admin Setup</a></li>
                  <li><a href="#"><a href="imagelist.php?ps=1">Sequencer</a></li>
                ...
                <section class="content" id='content-content-id' style='padding-right:5px;padding-left:5px;margin-top:-10px;' >
                  ...
                  <div class="nav-tabs-custom" id="nav-tabs-custom-id">
                        <ul class="nav nav-tabs">
                           ...
                           <li class="active"><a href="#personal" data-toggle="tab"><i class="fa fa-desktop" aria-hidden="true"></i>&nbsp; Icon</a></li>
                       </ul>
...
<?php
            $entrydata = "<form name='frmEdit' method='post' id='frmEdit' enctype='multipart/form-data'>
                                 ...
                                 <table class='table table-bordered table-condensed table-fixed  table-responsive' style='table-layout:fixed'>
                                                            <tr>
                                                              <td class='dvtCellLabel' style='border: 1px solid #ccc;'>Image Code:</td>
                                                              <td style='border: 1px solid #ccc;'><input type='text' class='form-control txt' value='$saveid' ></td>
                                                              <td class='dvtCellLabel' style='border: 1px solid #ccc;'> Image Name:</td>
                                                              <td style='border: 1px solid #ccc;'><input type='text' class='form-control txt inputs'  name='txt_A_imagename' id='txt_A_imagename'  value='$imagename' >
                       ...
                       if($update == "true" || ($insert=="true" && $_REQUEST['ID']==0))
                        ...
                        $entrydata.="  <button class='btn btn-danger inputs' style='margin-top:-5px;' name='btndanger' type='button'  onclick ='javascript:closeediting(\"imagelist.php?ps=1&pr=".$_SESSION['pr']."\");'>Close &nbsp;<i class='fa fa-close' aria-hidden='true'></i></button>";
                        
                        $entrydata.="  <button class='btn btn-primary inputs' style='margin-top:-5px;' name='btndanger' type='button'  onclick ='javascript:closeediting(\"imagelist.php?ps=1&objectid=".$_SESSION['objectid']."&frmPage_rowcount=".$_SESSION['frmPage_rowcount']."&txtsearch=".$_SESSION['txtsearch']."&frmPage_startrow=".$_SESSION['frmPage_startrow']."\");'>Back &nbsp;<i class='fa fa-backward' aria-hidden='true'></i></button>";
