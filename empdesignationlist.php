...
$grid->formName = "empdesignationlist.php";
$grid->inpage = $frmPage_startrow;
$grid->TableName = "tbl_designation";
$grid->SyncSession($grid);
$formlistname="empdesignationlist.php";
...
if($CHILDID !='' && isset($_REQUEST['DEL']) =='DELETE'){
     ...
     $Del_query="delete from tbl_designation where id='". $CHILDID."'";
...
/*if($_REQUEST['CHILDID'] !='' && $_REQUEST['DEL'] =='POST'){
     ...
     $Del_query="update tbl_designation set posted='YES' where id='". $_REQUEST['CHILDID']."'";
     $Del_Result = mysqli_query($con,$Del_query)   or die(mysqli_error()."<br>".$Del_query);
     $_REQUEST['CHILDID']="";
...
if($mysearch!=""){
    $addsql = " and (";
    $addsql .= "  designationname like '%".$mysearch."%'";
    $addsql .= ")";
}else{
    $addsql="";
}
$query = "SELECT * from tbl_designation where 1 $addsql  order by id DESC LIMIT $start_from, $record_per_page";
...
<h2 class='title'>Emp Designation</h2>
</section>
<?php
$entrydata = "<form name='frmEdit' id='frmEdit'  method='post'>
                 <section class='content' id='section-content-id' style='padding-right:5px;padding-left:5px;background-color:#fff;' >
                          <div class='box box-primary' id='box-content-id'  >
                            ...
                            <thead>
                             <tr bgcolor='#438EB9'>
                              <th style='width:20%;border-right:1px solid #ccc;cursor: pointer;'><font style='color:#fff'>Code</font></th>
                              <th style='width:60%;border-right:1px solid #ccc;cursor: pointer;'><font style='color:#fff'>Name</font></th>
                              <th style='width:20%;border-right:1px solid #ccc;'><font style='color:#fff'>Action</font></th>
                             </tr>
                             ";
         if($CHILDID==''){
                  $SQL   = "SELECT max(id) as id FROM tbl_designation";
            ...
            $entrydata .= " <tr id='tradd' name='tradd'>
                             ...
                             <td style='border: 1px solid #ccc;'><input type='text' class='form-control txt inputs' id='txt_A_designationname' name='txt_A_designationname'  value='' /></td>
    ...
    $page_query = "SELECT * from tbl_designation ORDER BY id DESC";
     ...
     while($row = mysqli_fetch_array($result))
     {
       ...
       if($row['id']==$CHILDID){
        ...
        $entrydata .= "<tr><td style='background-color:$colorbg;border: 1px solid #ccc;'><input type='text' class='form-control txt inputs' disabled value='".$row['id']."'/></td>
                       <td style='background-color:$colorbg;border: 1px solid #ccc;'><input type='text' class='form-control txt inputs' id='txt_A_designationname' name='txt_A_designationname' value='".$row['designationname']."' /></td>
       ...
       $entrydata .="<tr id='tr_".$row['id']."'  name='tr_".$row['id']."'>
                      ...
                      <td style='background-color:$colorbg;border:1px #D2D2D2 solid;'><font>&nbsp;&nbsp;&nbsp;&nbsp;".$row["designationname"]."</font></td>";
    ...
    if($page > 1)
    {
     echo "<a  href='empdesignationlist.php?pr=$pr&page=1' style='padding:8px 16px;border:1px solid #ccc;color:#333;'   >First</a>";
     echo "<a href='empdesignationlist.php?pr=$pr&page=".($page - 1)."' style='padding:8px 16px;border:1px solid #ccc;color:#333;'   ><<</a>";
    }
    for($i=$page; $i<=$end_loop; $i++)
    {
     ...
     echo "<a $x href='empdesignationlist.php?pr=$pr&page=".$i."' style='padding:8px 16px;border:1px solid #ccc;color:#333;'   ><font style='font-size: 13px;color:$font'>".$i."</font></a>";
    }
   // echo $page."/".$end_loop;
    if($page <= $end_loop)
    {
     echo "<a href='empdesignationlist.php?pr=$pr&page=".($page + 1)."' style='padding:8px 16px;border:1px solid #ccc;color:#333;'   >>></a>";
     echo "<a href='empdesignationlist.php?pr=$pr&page=".$total_pages."'style='padding:8px 16px;border:1px solid #ccc;color:#333;'   >Last</a>";
    }
   }
...
function refreshrecord(){
           var childid='';
           document.frmEdit.action='empdesignationlist.php?pr=<?php echo $pr;?>&CHILDID='+childid+'&mysearch='+document.getElementById('mysearch').value+'&page='+<?php echo $page; ?>;
           document.frmEdit.submit();
}
function updaterecord(childid){

    document.frmEdit.action='empdesignationlist.php?pr=<?php echo $pr;?>&mysearch='+document.getElementById('mysearch').value+'&CHILDID='+childid+'&page='+<?php echo $page; ?>;
   document.frmEdit.submit();
}
function deleterecord(childid){
        ...
        alertify.confirm("Are you sure you want to delete ?", function (e) {
         if (e) {
           document.frmEdit.action='empdesignationlist.php?pr=<?php echo $pr;?>&DEL=DELETE&CHILDID='+childid+'&page='+<?php echo $page; ?>;
...
function postrecord(childid){
        ...
        alertify.confirm("Are you sure you want to Post ?", function (e) {
         if (e) {
           document.frmEdit.action='empdesignationlist.php?pr=<?php echo $pr;?>&DEL=POST&CHILDID='+childid+'&page='+<?php echo $page; ?>;
...
function editingrecord(){
       ...
       var txt_A_designationname=document.getElementById('txt_A_designationname');
       if(txt_A_designationname){
          if ((txt_A_designationname.value==null)||(txt_A_designationname.value=="")){
               alertify.alert("Enter Look Type", function () {
               txt_A_designationname.focus();
                         ...
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                alertify.success("Record Saved");
                                 document.frmEdit.action='empdesignationlist.php?pr=<?php echo $pr;?>&page='+<?php echo $page; ?>;
                                 window.setTimeout(function() { document.frmEdit.submit(); }, 800);

                               }else if(s1.toString() == s3.toString()){
                                alertify.success("Record Updated");
                                document.frmEdit.action='empdesignationlist.php?pr=<?php echo $pr;?>&dr=edit&ID='+document.getElementById('mode').value+'&page='+<?php echo $page; ?>;
                                window.setTimeout(function() { document.frmEdit.submit(); }, 800);
                               }else{
                                alertify.error(s1);
                               }
...
function filtersublist(objEvent){
                     ...
                     if (iKeyCode==13) {
                         window.location.href='empdesignationlist.php?pr=<?php echo $pr;?>&mysearch='+document.getElementById('mysearch').value+'&page='+<?php echo $page; ?>;
                         return false;
                     }
}
