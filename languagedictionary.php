...
$pr = $_SESSION['pr'];
//print_r($_REQUEST);
$language_objectid = isset($_REQUEST['looktype']) ? $_REQUEST['looktype'] : '';
...
$grid->formName = "languagedictionary.php";
$grid->inpage = $frmPage_startrow;
$grid->TableName = "tbl_languagedictionary";
$grid->SyncSession($grid);
$formlistname="languagedictionary.php";
...
if($CHILDID !='' && isset($_REQUEST['DEL']) =='DELETE'){
     ...
     $Del_query="delete from tbl_languagedictionary where id='". $CHILDID."'";
...
/*if($_REQUEST['CHILDID'] !='' && $_REQUEST['DEL'] =='POST'){
     ...
     $Del_query="update tbl_languagedictionary set posted='YES' where id='". $_REQUEST['CHILDID']."'";
     $Del_Result = mysqli_query($con,$Del_query)   or die(mysqli_error()."<br>".$Del_query);
     $_REQUEST['CHILDID']="";
...
if($mysearch!=""){
    $addsql = " and (";
    $addsql .= "  lable like '%".$mysearch."%'";
    $addsql .= "  english like '%".$mysearch."%'";
    $addsql .= "  german like '%".$mysearch."%'";
    $addsql .= "  arabic like '%".$mysearch."%'";
    $addsql .= ")";
}else{
    $addsql="";
}

if($language_objectid!=""){
	$addsql.=" and objectid='".$language_objectid."'";
}
$query = "SELECT * from tbl_languagedictionary where 1 $addsql  order by id DESC LIMIT $start_from, $record_per_page";
...
$entrydata = "<form name='frmEdit' id='frmEdit'  method='post'>
                          ...
                          <div class='box box-primary' id='box-content-id'  >
                            <div class='box-body no-padding' id='box-body-id' style='overflow:hidden;' >
                             <table class='table table-condensed ' style='margin-top:3px;' border='0'>
                                             ...
                                             <tr>
                                             <td width='40%'>".GetGroup($language_objectid)."</td>
                            ...
                            <thead>
                             <tr bgcolor='#438EB9'>
                              <th style='width:20%;border-right:1px solid #ccc;cursor: pointer;'><font style='color:#fff'>Lable</font></th>
                              <th style='width:20%;border-right:1px solid #ccc;cursor: pointer;'><font style='color:#fff'>English</font></th>
                              <th style='width:20%;border-right:1px solid #ccc;cursor: pointer;'><font style='color:#fff'>Arabic</font></th>
                              <th style='width:20%;border-right:1px solid #ccc;cursor: pointer;'><font style='color:#fff'>German</font></th>
                              <th style='width:10%;border-right:1px solid #ccc;'><font style='color:#fff'>Action</font></th>
                             </tr>";

       $entrydata .= "  </thead>
                             <input type='hidden' name='objectid' id='objectid' class=textboxcombo value='".$_SESSION['objectid']."'>
    ...
    $page_query = "SELECT * from tbl_languagedictionary where 1 $addsql ORDER BY id DESC";
    ...
    $i=1;
     while($row = mysqli_fetch_array($result))
     {
       ...
       if($row['id']==$CHILDID){
        ...
        $colorbg ="#F2F2F2";
        $entrydata .= "<tr>
        			   <td style='background-color:$colorbg;border: 1px solid #ccc;'>".$row['lable']."</td>
        			   <td style='background-color:$colorbg;border: 1px solid #ccc;'>
        			   <input type='text' class='form-control txt inputs' id='txt_A_english' name='txt_A_english' value='".$row['english']."'/></td>
        			   <td style='background-color:$colorbg;border: 1px solid #ccc;'>
        			   <input type='text' class='form-control txt inputs' id='txt_A_arabic' name='txt_A_arabic' value='".$row['arabic']."' style='text-align:right;'/></td>
                       <td style='background-color:$colorbg;border: 1px solid #ccc;'>
                       <input type='text' class='form-control txt inputs' id='txt_A_german' name='txt_A_german' value='".$row['german']."' /></td>
       ...
       $entrydata .="<tr id='tr_".$row['id']."'  name='tr_".$row['id']."'>
                      ...
                      <td style='background-color:$colorbg;border:1px #D2D2D2 solid;'><font>&nbsp;".$row["lable"]."</font></td>
                      <td style='background-color:$colorbg;border:1px #D2D2D2 solid;'><font>&nbsp;".$row["english"]."</font></td>
                      <td style='background-color:$colorbg;border:1px #D2D2D2 solid;text-align:right;'><font>&nbsp;".$row["arabic"]."</font></td>
                      <td style='background-color:$colorbg;border:1px #D2D2D2 solid;'><font>&nbsp;".$row["german"]."</font></td>";
    ...
    if($page > 1)
    {
     echo "<a  href='languagedictionary.php?objectid=".$_SESSION['objectid']."&looktype=$language_objectid&page=1' style='padding:8px 16px;border:1px solid #ccc;color:#333;'   >First</a>";
     echo "<a href='languagedictionary.php?objectid=".$_SESSION['objectid']."&looktype=$language_objectid&page=".($page - 1)."' style='padding:8px 16px;border:1px solid #ccc;color:#333;'   ><<</a>";
    }
    for($i=$page; $i<=$end_loop; $i++)
    {
     ...
     echo "<a $x href='languagedictionary.php?objectid=".$_SESSION['objectid']."&looktype=$language_objectid&page=".$i."' style='padding:8px 16px;border:1px solid #ccc;color:#333;'   ><font style='font-size: 13px;color:$font'>".$i."</font></a>";
    }
   // echo $page."/".$end_loop;
    if($page <= $end_loop)
    {
     echo "<a href='languagedictionary.php?objectid=".$_SESSION['objectid']."&looktype=$language_objectid&page=".($page + 1)."' style='padding:8px 16px;border:1px solid #ccc;color:#333;'   >>></a>";
     echo "<a href='languagedictionary.php?objectid=".$_SESSION['objectid']."&looktype=$language_objectid&page=".$total_pages."'style='padding:8px 16px;border:1px solid #ccc;color:#333;'   >Last</a>";
    }
   }
...
function refreshrecord(){
           var childid='';
           document.frmEdit.action='languagedictionary.php?pr=<?php echo $pr;?>&CHILDID='+childid+'&mysearch='+document.getElementById('mysearch').value+'&page='+<?php echo $page; ?>;
           document.frmEdit.submit();
}
function updaterecord(childid){
	//document.getElementById('tradd').style.display = 'none'; 
    document.frmEdit.action='languagedictionary.php?pr=<?php echo $pr;?>&mysearch='+document.getElementById('mysearch').value+'&CHILDID='+childid+'&page='+<?php echo $page; ?>;
   document.frmEdit.submit();
}
function deleterecord(childid){
        ...
        alertify.confirm("Are you sure you want to delete ?", function (e) {
         if (e) {
           document.frmEdit.action='languagedictionary.php?pr=<?php echo $pr;?>&DEL=DELETE&CHILDID='+childid+'&page='+<?php echo $page; ?>;
...
function postrecord(childid){
        ...
        alertify.confirm("Are you sure you want to Post ?", function (e) {
         if (e) {
           document.frmEdit.action='languagedictionary.php?pr=<?php echo $pr;?>&DEL=POST&CHILDID='+childid+'&page='+<?php echo $page; ?>;
                         ...
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                alertify.success("Record Saved");
                                 document.frmEdit.action='languagedictionary.php?pr=<?php echo $pr;?>&page='+<?php echo $page; ?>;
                                 window.setTimeout(function() { document.frmEdit.submit(); }, 800);

                               }else if(s1.toString() == s3.toString()){
                                alertify.success("Record Updated");
                                document.frmEdit.action='languagedictionary.php?pr=<?php echo $pr;?>&dr=edit&ID='+document.getElementById('mode').value+'&page='+<?php echo $page; ?>;
                                window.setTimeout(function() { document.frmEdit.submit(); }, 800);
                               }else{
                                alertify.error(s1);
                               }
...
function filtersublist(objEvent){
                     ...
                     if (iKeyCode==13) {
                         window.location.href='languagedictionary.php?pr=<?php echo $pr;?>&mysearch='+document.getElementById('mysearch').value+'&page='+<?php echo $page; ?>;
                         return false;
                     }
}
...
function GetGroup($lookcode){ 
	global $con;
         $CMB = "<span style='margin-top:5px;float:left'>Select Form :&nbsp;&nbsp;</span>";
         ...
         $CMB .= "<option value=''>Select</option>";
         $SEL =  "select objectid,tbl_objectmaster.objectname from tbl_languagedictionary left join tbl_objectmaster on tbl_languagedictionary.objectid = tbl_objectmaster.id
where 1 group by tbl_languagedictionary.objectid";
         $RES = mysqli_query($con,$SEL);
         while ($ARR = mysqli_fetch_array($RES)) {
                $SEL = "";
                if(strtoupper($lookcode) == strtoupper($ARR['objectid'])){ $SEL =  "SELECTED";}
                if($ARR['objectid'] == 'Menus')
                $CMB .= "<option value='".$ARR['objectid']."' $SEL >".$ARR['objectid']."</option>";
                else
                $CMB .= "<option value='".$ARR['objectid']."' $SEL >".$ARR['objectname']."</option>";
         }
         $CMB .= "</select>";
         return $CMB;
