...
if($_REQUEST['ID'] != "0") {
             ...
             $SQL = " Select * from in_sequencer where id='".$_REQUEST['ID']."'";
             $SQLRes =  mysqli_query($con,$SQL) or die(mysqli_error()."<br>".$SQL);
              if(mysqli_num_rows($SQLRes)>=1){
                while($loginResultArray   = mysqli_fetch_array($SQLRes)){
                   ...
                   $TABLENAME = $loginResultArray['TABLENAME'];
                   $LASTNUMBER=  $loginResultArray['LASTNUMBER'];
                  }
              }
           }else{
              $mode="";
              $saveid = GetLastSqeID("in_sequencer");
              $TABLENAME = $LASTNUMBER = "";
...
if(isset($_REQUEST['dr'])=='view'){
   $edit="none";
   $view="inline";
   $title="Viewing Sequencer : ".$TABLENAME."";
}else if(isset($_REQUEST['dr'])=='edit'){
      $edit="inline";
      $view="none";
      $title="Editing Sequencer : ".$TABLENAME."";
}else{
      $edit="inline";
      $view="none";
      $title="Adding New Sequencer";
}
       ...
       insertfunction(get(document.frmEdit),action)
                   ...
                   function stateChangedsave()
                   {
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                 ...
                                 window.location.href='editsequencer.php?dr=edit&ID='+document.getElementById('saveid').value;
                                });
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='editsequencer.php?dr=edit&ID='+document.getElementById('mode').value;
                    ...
                    function stateChangedsavenew()
                   {
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                               alertify.alert("Record Saved", function () {
                                 window.location.href='editsequencer.php?dr=add&ID=0';
                                });
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='editsequencer.php?dr=add&ID=0';
                    ...
                    function stateChangedsaveclose()
                   {
                         ...
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                ...
                                window.location.href='sequencer.php?ID=0';
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='sequencer.php';
...
<body class="hold-transition sidebar-mini">
         ...
         <section class="content-header">
                 ...
                 <a class="pull-left" href="sequencer.php?frmPage_rowcount=<?echo $_SESSION['frmPage_rowcount']; ?>&txtsearch=<?echo $_SESSION['txtsearch']; ?>&frmPage_startrow=<?echo $_SESSION['frmPage_startrow'];?>" data-toggle="tooltip" data-placement="right" title="Back to Home"><i class='fa fa-backward'></i></a>
                 <h2 class="title">&nbsp;&nbsp;<?echo $title; ?></h2>
<!--
                 <ol class='breadcrumb'>
                  <li><a href="#"><a href="blank.php" >Admin Setup</a></li>
                  <li><a href="#"><a href="sequencer.php?ps=1">Sequencer</a></li>
                ...
                <section class="content" id='content-content-id' style='padding-right:5px;padding-left:5px;margin-top:-10px;' >
                  ...
                  <div class="nav-tabs-custom" id="nav-tabs-custom-id">
                        <ul class="nav nav-tabs">
                           ...
                           <li class="active"><a href="#personal" data-toggle="tab"><i class="fa fa-desktop" aria-hidden="true"></i>&nbsp; Sequencer</a></li>
                       </ul>
...
<?
            $entrydata = "<form name='frmEdit' method='post' id='frmEdit' enctype='multipart/form-data'>
                                 ...
                                 <table class='table table-bordered table-condensed table-fixed  table-responsive' style='table-layout:fixed'>
                                                            <tr>
                                                              <td class='dvtCellLabel' style='border: 1px solid #ccc;'>Table Name:</td>
                                                              <td style='border: 1px solid #ccc;'><input type='text' class='form-control txt'   name='txt_A_TABLENAME' id='txt_A_TABLENAME'  value='$TABLENAME' ></td>
                                                              <td class='dvtCellLabel' style='border: 1px solid #ccc;'>Last Number:</td>
                                                              <td style='border: 1px solid #ccc;'><input type='text' onkeypress='return AllowNumeric1(event);' min='10000' class='form-control txt inputs'  name='txt_A_LASTNUMBER' id='txt_A_LASTNUMBER'  value='$LASTNUMBER' >
                                        ...
                                        <button class='btn btn-success inputs' name='btnsuccess' type='button'  onclick ='javascript:editingrecord(\"save\");'>Save </font>&nbsp;<i class='fa fa-save' aria-hidden='true'></i></button>
                                        <button class='btn btn-info inputs' name='btninfo' type='button' onclick ='javascript:editingrecord(\"savenew\");'>Save & New &nbsp; <i class='fa fa-save' aria-hidden='true'></i>&nbsp;&nbsp;<i class='fa fa-plus' aria-hidden='true'></i></button>
                                        <button class='btn btn-warning inputs' name='btnwarning' type='button'  onclick ='javascript:editingrecord(\"saveclose\");'>Save & Close &nbsp;<i class='fa fa-save' aria-hidden='true'></i>&nbsp;&nbsp;<i class='fa fa-close' aria-hidden='true'></i></button>
                                        <button class='btn btn-danger inputs' name='btndanger' type='button'  onclick ='javascript:cancleediting(\"sequencer.php\");'>Close &nbsp;<i class='fa fa-close' aria-hidden='true'></i></button>
                                        </div>
                                         </div></form> ";
...
function loadpage(i){
   ...
   frame.src='emp_documents1.php?entitytype=Company&ID='+<?echo $_REQUEST['ID']; ?>;
   frame.load();
   }
}
