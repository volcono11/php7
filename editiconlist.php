...
if($_REQUEST['ID'] != "0") {
             ...
             $SQL = " Select * from tbl_icons where id='".$_REQUEST['ID']."'";
             $SQLRes =  mysqli_query($con,$SQL) or die(mysqli_error()."<br>".$SQL);
              if(mysqli_num_rows($SQLRes)>=1){
                while($loginResultArray   = mysqli_fetch_array($SQLRes)){
                   ...
                   $iconcode = htmlspecialchars($loginResultArray['iconcode']);
                   $iconname=  $loginResultArray['iconname'];
                  }
              }
           }else{
              $mode="";
              $saveid =  GetLastSqeID("tbl_icons");
              $iconcode = $iconname = "";
...
if(isset($_REQUEST['dr'])=='view'){
   $edit="none";
   $view="inline";
   $title="Viewing : ".$iconname."";
}else if(isset($_REQUEST['dr'])=='edit'){
      $edit="inline";
      $view="none";
      $title="Editing : ".$iconname."";
}else{
      $edit="inline";
      $view="none";
      $title="Adding New Icon";
}
...
function editingrecord(action)
{
       ...
       var txt_A_TABLENAME=document.getElementById('txt_A_iconcode');
       if(txt_A_TABLENAME){
          if ((txt_A_TABLENAME.value==null)||(txt_A_TABLENAME.value=="")){
               alertify.alert("Enter Icon Code", function () {
       ...
       var txt_A_LASTNUMBER=document.getElementById('txt_A_iconname');
       if(txt_A_LASTNUMBER){
       ...
       insertfunction(get(document.frmEdit),action)
                   ...
                   function stateChangedsave()
                   {
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                 ...
                                 window.location.href='editiconlist.php?dr=edit&ID='+document.getElementById('saveid').value;
                                });
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='editiconlist.php?dr=edit&ID='+document.getElementById('mode').value;
                    ...
                    function stateChangedsavenew()
                   {
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                               alertify.alert("Record Saved", function () {
                                 window.location.href='editiconlist.php?dr=add&ID=0';
                                });
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='editiconlist.php?dr=add&ID=0';
                    ...
                    function stateChangedsaveclose()
                   {
                         ...
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                ...
                                window.location.href='iconlist.php?ID=0';
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='iconlist.php';
...
<body class="hold-transition sidebar-mini">
         ...
         <section class="content-header">
                 ...
                 <a class="pull-left" href="iconlist.php?objectid=<?php echo $_SESSION['objectid']; ?>&pr=<?php echo $_SESSION['pr']; ?>&txtsearch=<?php echo $_SESSION['txtsearch']; ?>" data-toggle="tooltip" data-placement="right" title="Back to Home"><i class='fa fa-backward'></i></a>
...
<?php
            $entrydata = "<form name='frmEdit' method='post' id='frmEdit' enctype='multipart/form-data'>
                                 ...
                                 <table class='table table-bordered table-condensed table-fixed  table-responsive' style='table-layout:fixed'>
                                                            <tr>
                                                              <td class='dvtCellLabel' style='border: 1px solid #ccc;'>Icon Code:</td>
                                                              <td style='border: 1px solid #ccc;'><input type='text' class='form-control txt'   name='txt_A_iconcode' id='txt_A_iconcode'  value='$iconcode' ></td>
                                                              <td class='dvtCellLabel' style='border: 1px solid #ccc;'> Icon Name:</td>
                                                              <td style='border: 1px solid #ccc;'><input type='text' class='form-control txt inputs'  name='txt_A_iconname' id='txt_A_iconname'  value='$iconname' >
                       ...
                       if($update == "true" || ($insert=="true" && $_REQUEST['ID']==0))
                        ...
                        $entrydata.="  <button class='btn btn-danger inputs' style='margin-top:-5px;' name='btndanger' type='button'  onclick ='javascript:closeediting(\"iconlist.php?ps=1&pr=".$_SESSION['pr']."\");'>Close &nbsp;<i class='fa fa-close' aria-hidden='true'></i></button>";
                        
                        $entrydata.="  <button class='btn btn-primary inputs' style='margin-top:-5px;' name='btndanger' type='button'  onclick ='javascript:closeediting(\"iconlist.php?ps=1&objectid=".$_SESSION['objectid']."&frmPage_rowcount=".$_SESSION['frmPage_rowcount']."&txtsearch=".$_SESSION['txtsearch']."&frmPage_startrow=".$_SESSION['frmPage_startrow']."\");'>Back &nbsp;<i class='fa fa-backward' aria-hidden='true'></i></button>";
