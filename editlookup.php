             ...
             $SQLRes =  mysqli_query($con,$SQL) or die(mysqli_error()."<br>".$SQL);
              if(mysqli_num_rows($SQLRes)>=1){
                while($loginResultArray   = mysqli_fetch_array($SQLRes)){
                   ...
                   //$modulename=  $loginResultArray['modulename'];
                   $remarks=    $loginResultArray['remarks'];
                  }
              }
           }else{
             $mode="";
             $saveid = $lookcode =GetLastSqeID('in_lookup');
             $looktype =  $remarks = "";
}
if(isset($_REQUEST['dr'])=='view'){
   $edit="none";
   $view="inline";
   $title="Viewing Lookup : $looktype";
}else if(isset($_REQUEST['dr'])=='edit'){
      $edit="inline";
      $view="none";
      $title="Editing Lookup : $looktype";
}else{
      $edit="inline";
      $view="none";
      $title="Adding New Lookup";
}
       ...
       insertfunction(parameter,action)
                   ...
                   function stateChangedsave()
                   {
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                 ...
                                 window.location.href='editlookup.php?dr=edit&ID='+document.getElementById('saveid').value;
                                });
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='editlookup.php?dr=edit&ID='+document.getElementById('mode').value;
                    ...
                    function stateChangedsavenew()
                   {
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                 alertify.alert("Record Saved", function () {
                                 window.location.href='editlookup.php?dr=add&ID=0';
                                });
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='editlookup.php?dr=add&ID=0';
                    ...
                    function stateChangedsaveclose()
                   {
                         ...
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                ...
                                window.location.href='lookup.php?ID=0';
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
...
<body class="hold-transition sidebar-mini">
         ...
         <section class="content-header">
                 ...
                 <a class="pull-left" href="lookup.php?ps=1&pr=<?php echo $_SESSION['pr'];?>" data-toggle="tooltip" data-placement="right" title="Back to Company Setup"><i class='fa fa-backward'></i></a>
                 <h2 class="title">&nbsp;&nbsp;<?php echo $title; ?></h2>

                 <!--<ol class='breadcrumb'>
                  <li><a href="#"><a href="blank.php" >Admin Setup</a></li>
                  <li><a href="#"><a href="lookup.php?ps=1">Lookup</a></li>
                ...
                <section class="content" id='content-content-id' style='padding-right:5px;padding-left:5px;margin-top:-10px;' >
                  ...
                  <div class="nav-tabs-custom" id="nav-tabs-custom-id">
                        <ul class="nav nav-tabs">
                           ...
                           <li class="active"><a href="#personal" data-toggle="tab"><i class="fa fa-desktop" aria-hidden="true"></i>&nbsp; Lookup</a></li>
                       </ul>
...
<?php
         ...
         $entrydata = "<form name='frmEdit' method='post' id='frmEdit' enctype='multipart/form-data'>
                                 ...
                                 <table class='table table-bordered table-condensed table-fixed  table-responsive' style='table-layout:fixed'>
                                                            <tr>
                                                              <td class='dvtCellLabel' style='border: 1px solid #ccc;'>Look Code:</td>
                                                              <td style='border: 1px solid #ccc;'><input type='text' class='form-control txt inputs' readonly   name='txt_A_lookcode' id='txt_A_lookcode'  value='$lookcode' ></td>
                                                              <td class='dvtCellLabel' style='border: 1px solid #ccc;'>Look Type:</td>
                                                              <td style='border:1px solid #ccc;'><input type='text' class='form-control txt inputs' name='txt_A_looktype' id='txt_A_looktype' value='$looktype' style='text-transform:uppercase'></td>
                                                                ...
                                                                <input type='hidden' name='txt_A_lookname' class=textboxcombo id='txt_A_lookname' value='XX'>
                                                            </tr>
                                                            </table>
                       ...
                       if($update == "true" || $insert=="true")
                        ...
                        $entrydata.="  <button class='btn btn-danger inputs' style='margin-top:-5px;' name='btndanger' type='button'  onclick ='javascript:closeediting(\"lookup.php?ps=1&pr=".$_SESSION['pr']."\");'>Close &nbsp;<i class='fa fa-close' aria-hidden='true'></i></button>
                                        </div>";
                        $entrydata.= "</form>  ";
echo  $entrydata;
?>
...
<?php
function GetModule($roles){
	global$con;
             $SQLsub1   = "select lookcode,lookname from in_lookup where looktype='MODULENAME' and lookname<>'XX' order by id";
             ...
             while($loginResultArraySub1   = mysqli_fetch_array($SQLResSub1)){
                          $selected="";
                          $val=  $loginResultArraySub1['lookcode'];
                          if($roles !=""){
                             /*$Htemp     = split(",",$roles);
                             for($ii=0 ;$ii < count($Htemp); $ii++){
                                if($Htemp[$ii]==$loginResultArraySub1['lookcode']) $selected ='checked="checked"';
                             }*/
                             $selected = "";
                          ...
                          $mycontrol = "<input type='checkbox' id='chk_A_modulename' name='chk_A_modulename[]' value='".$loginResultArraySub1['lookcode']."' $selected/>&nbsp;&nbsp;" . htmlspecialchars(trim($loginResultArraySub1['lookname'])) . "&nbsp;&nbsp;";
