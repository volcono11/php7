     ...
     if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
     {
           ...
           if(s1.toString() == s2.toString()){
             alertify.alert("Record Saved", function () {
              document.frmChildEdit.action='emp_modules.php?PARENTID='+document.getElementById('txt_A_userid').value;
...
function updateChildrecord(childid){
    ...
    document.frmChildEdit.action='emp_modules.php?CHILDID='+childid+'&PARENTID='+document.getElementById('txt_A_userid').value;
    document.frmChildEdit.submit();
}
...
function deleteChildrecord(childid){
        ...
        alertify.confirm("Are you sure you want to delete ? ", function (e) {
         if (e) {
           document.frmChildEdit.action='emp_modules.php?DEL=DELETE&CHILDID='+childid+'&PARENTID='+document.getElementById('txt_A_userid').value;
           document.frmChildEdit.submit();
         } else {
            return;
         }
...
$frmPage_startrow = isset($_REQUEST['frmPage_startrow']) ? $_REQUEST['frmPage_startrow'] : '';
    ...
    $formlistname = "emp_modules.php";

    $grid = new MyPHPGrid('frmPage');

    $grid->formName = "emp_modules.php";

    $grid->inpage = $frmPage_startrow;

    $grid->TableNameChild = "tbl_usermodule";
...
if($CHILDID !='' && $DEL =='DELETE'){
        mysqli_query($con,"delete from tbl_usermodule where id='". $CHILDID."'");
        $CHILDID ="";

}
                                  
        $sql_1 = "select * from tbl_usermodule where id='".$CHILDID."'";
        $res_1 = mysqli_query($con,$sql_1);
                                  
		if(mysqli_num_rows($res_1)>=1){
		    $arr_1 = mysqli_fetch_array($res_1);
		    $modulecode=$arr_1['modulecode'];
	        $usergroup=$arr_1['usergroup'];
		}
		else{
			$modulecode = $usergroup = "";
		}
        ...
        $entrydata = "<div class='table-responsive no-padding'>
            <form name='frmChildEdit' method='post' id='frmChildEdit' autocomplete='off' enctype='multipart/form-data'>
                <table class='table table-bordered table-condensed table-fixed  table-responsive' style='table-layout:fixed'>
                    <tr>
                        <td class='dvtCellLabel' style='border: 1px solid #ccc;width:13%;'>Module $mandatory</td>
                        <td style='border: 1px solid #ccc;'>".GetModules($modulecode)."</td>
                        <td class='dvtCellLabel' style='border: 1px solid #ccc;width:13%;'>User Group $mandatory</td>
                        <td style='border: 1px solid #ccc;'>
                        ".GetUsergroup($usergroup)."
                        </td>
                        <td style='border: 1px solid #fff;width:16%;'>
                        ".$Save_button."
                        <input type='hidden' class=textboxcombo name='txt_A_userid' id='txt_A_userid' value='".$PARENTID."'>
...
if(isset($_REQUEST['search'])!=""){
	$addsql = " and (";
	$addsql .= " tbl_usergroup.usergroup like '%".$_REQUEST['search']."%'";
	$addsql .= ")";
}

$list_sql = "select tbl_usermodule.*,A.usergroup as role,B.lookname as module from tbl_usermodule left join tbl_usergroup as A on A.id=tbl_usermodule.usergroup left join in_lookup as B on B.lookcode=tbl_usermodule.modulecode where tbl_usermodule.userid='".$PARENTID."' $addsql";
...
$result = mysqli_query($con,$sql) or die(mysqli_error());
        ...
        $entrydatatable.= "<th class='bg-light-blue' style='width:15%;text-align:left;border-bottom:1px #2F3C43 solid;border-top:1px #fff solid;color:#FFFFFF'>Module</th>";
        $entrydatatable.= "<th class='bg-light-blue' style='width:10%;text-align:left;border-bottom:1px #2F3C43 solid;border-top:1px #fff solid;color:#FFFFFF'>role</th>";
		...
		while($loginResultArrayChild   = mysqli_fetch_array($result)){
        	...
        	$entrydatatable.= "<td style='background-color:$colorbg;color:$colorfc;border:1px #ccc solid;'>" . $loginResultArrayChild['module']. "</td>";
        	$entrydatatable.= "<td style='background-color:$colorbg;color:$colorfc;border:1px #ccc solid;'>" . $loginResultArrayChild['role']. "</td>";
        	if($update == "true")
...
function GetModules($roles){
	global $con;
    $CMB = " <select name='cmb_A_modulecode'  id='cmb_A_modulecode' class='form-control select2'>";    
    $CMB .= "<option value=''>Select</option>";
	$SEL =  "Select lookcode,lookname from in_lookup where looktype='MODULES' and lookname<>'XX'";
...
function GetUsergroup($usergroup){
	global $con;
    $CMB = " <select name='cmb_A_usergroup'  id='cmb_A_usergroup' class='form-control select'>";    
    $CMB .= "<option value=''>Select</option>";
	$SQL =  "Select id as lookcode,usergroup as lookname from tbl_usergroup order by 2";
	$RES = mysqli_query($con,$SQL);
	while ($ARR = mysqli_fetch_array($RES)) {
	   $SEL = "";
	   if($usergroup == $ARR['lookcode']){ $SEL =  "SELECTED";}
	   $CMB .= "<option value='".trim($ARR['lookcode'])."' $SEL >".trim($ARR['lookname'])."</option>";
	}
	$CMB .= "</select>";
	return $CMB;
