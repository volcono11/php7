...
function getGadgets(cattype){
      ...
      var url="combofunctions_hr.php?level=3&categorytype="+cattype;
...
function stateChangedcombo(){
       if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
       {
             ...
             document.getElementById('getgaditem').innerHTML=s1;

       }
}
function updateChildrecord(childid){

    document.frmChildEdit.action='upload_documents.php?entitytype='+document.getElementById('txt_A_entitytype').value+'&id1=<?echo $_GET['id1']; ?>&CHILDID='+childid+'&ID='+document.getElementById('txt_A_staffid').value;
...
function deleteChildrecord(childid){
        ...
        alertify.confirm("Are you sure you want to delete ?", function (e) {
         if (e) {
           document.frmChildEdit.action='upload_documents.php?entitytype='+document.getElementById('txt_A_entitytype').value+'&id1=<?echo $_GET['id1']; ?>&DEL=DELETE&CHILDID='+childid+'&ID='+document.getElementById('txt_A_staffid').value;
           document.frmChildEdit.submit();
         } else {
            return;
         }
...
function deletedocChildrecord(childid){
        ...
        alertify.confirm("Are you sure you want to delete this attachment ?", function (e) {
         if (e) {
           document.frmChildEdit.action='upload_documents.php?entitytype='+document.getElementById('txt_A_entitytype').value+'&id1=<?echo $_GET['id1']; ?>&DOCCHILDID='+childid+'&ID='+document.getElementById('txt_A_staffid').value;
           document.frmChildEdit.submit();
         } else {
            return;
         }
...
function canceleditingChildrecord(){
    ...
    document.frmChildEdit.action = 'upload_documents.php?ID='+document.getElementById('txt_A_staffid').value;
    document.frmChildEdit.submit();
}
function addSlot()
        {
                         ...
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                 alertify.alert("Record Saved", function () {
                                  document.frmChildEdit.action='upload_documents.php?entitytype='+document.getElementById('txt_A_entitytype').value+'&id1=<?echo $_GET['id1']; ?>&ID='+document.getElementById('txt_A_staffid').value;
                                  document.frmChildEdit.submit();
                                 });
...
function searchPaging(){
       ...
       document.frmChildEdit.action='upload_documents.php?entitytype='+document.getElementById('txt_A_entitytype').value+'&id1=<?echo $_GET['id1']; ?>&search='+document.getElementById('txtsearch').value+'&ID='+document.getElementById('txt_A_staffid').value;
       document.frmChildEdit.submit();
}
function refreshPaging(){

       document.frmChildEdit.action='upload_documents.php?entitytype='+document.getElementById('txt_A_entitytype').value+'&ID='+document.getElementById('txt_A_staffid').value;
       document.frmChildEdit.submit();
...
<section class="content" id='content-content-id' style='padding-right:5px;padding-left:5px;' >
   <div class="box-body" id='box-body-id' style='background-color:#ffffff;'>
          <div class='table-responsive'>
                  <?
/*                          $staffid=$_REQUEST['ID'];
                           $contractstatus = GetContractStatus($_REQUEST['ID']);
                           ...
                           $arguments = array('id_field' => $id_field, 'fieldNames' => $fieldNames, 'fieldSizes' => $fieldSizes,
                                        ...
                                        'selectedlistingpage'=>$_SESSION['frmPage_startrow']);*/
			...
			$grid->TableNameChild = "e_documents";
			...
			if($CHILDID !='' && $DEL !='DELETE'){
			        ...
			        $SEL12 = "Select *,DATE_FORMAT(issuedate,'%d-%m-%Y') as issuedate,DATE_FORMAT(expirydate,'%d-%m-%Y') as expirydate from e_documents where id ='".$CHILDID."' and entitytype='".$entitytype."'";
            ...
            if($CHILDID !='' && $DEL =='DELETE'){
                    ...
                    $Del_query="delete from e_documents where id='". $CHILDID."' and entitytype='".$entitytype."'";
                    echo UserLog("e_documents",$CHILDID,$Del_query,"DELETE");
            ...
            if($DOCCHILDID !=''){
                    ...
                    $Del_query="delete from e_attachments where id='". $_REQUEST['DOCCHILDID']."'";
                    echo UserLog("e_attachments",$CHILDID,$Del_query,"DELETE");
                    $Del_Result = mysqli_query($con,$Del_query)   or die(mysqli_error()."<br>".$Del_query);
                    $_REQUEST['DOCCHILDID']="";
                            ...
                            <tr>
                              ...
                              <td style='border: 1px solid #ccc;'><a href='javascript:editingChildrecord();'><img src='ico/save.png' title='Save' width='20' height='20'></a>
                                    &nbsp;&nbsp;<a href='?entitytype=".$entitytype."&ID=".$PARENTID."'><img src='ico/cancel.png' title='Cancel' width='20' height='20'></a>
                                     ...
                                     <input type='hidden' class=textboxcombo name='txt_A_staffid' id='txt_A_staffid' value='".$PARENTID."'>
	...
	/*if($entitytype=="Profitcenter Documents" && stripos(json_encode($_SESSION['role']),'OPERATIONS MANAGER') != true && strtoupper($contractstatus)!='INACTIVE'){
        echo $entrydata;
	}
	if($entitytype=="Ticket Documents" && stripos(json_encode($_SESSION['role']),'SITE INCHARGE') == true){
		echo $entrydata;
	}
	if($entitytype=="Supplier Documents" && stripos(json_encode($_SESSION['role']),'SERVICE COORDINATOR') == true){
		echo $entrydata;
	}
	if($entitytype=="Purchase Documents" && stripos(json_encode($_SESSION['role']),'SALES COORDINATOR') == true){
		echo $entrydata;
	}*/
	if($entitytype=="COMPANY_DOCUMENTS"){
		echo $entrydata;
         ...
         $addsql="";
         if(stripos(json_encode($_SESSION['role']),'SITE INCHARGE') == true || stripos(json_encode($_SESSION['role']),'FACILITY MANAGER') == true ){
$addsql = " and lookcode<>'46001'";
} 
         ...
         $rows1=mysqli_num_rows(mysqli_query($con,"SELECT *,DATE_FORMAT(issuedate,'%d-%m-%Y') as issuedate,DATE_FORMAT(expirydate,'%d-%m-%Y') as expirydate
                FROM e_documents,in_lookup where e_documents.documenttype=in_lookup.lookcode and staffid='".$PARENTID."' and entitytype='".$entitytype."' $addsql"));

         echo "<div class='box' style='border:0px;padding:0px;'>
                <div class='box-tools pull-left '>
                     <input type='text' name='txtsearch' onkeypress='return filtersublist(event);' id=txtsearch class='form-control' style='height:24px;border: 1px solid #ccc;width:200px;' placeholder='Search..' value=".isset($_REQUEST['search']).">
                </div>
                <div class='box-tools pull-left '>
         ...
         echo "</ul>
                 ...
                 $sql = "SELECT e_documents.id as id,entitytype,lookname,documentreference,DATE_FORMAT(issuedate,'%d-%m-%Y') as issuedate,DATE_FORMAT(expirydate,'%d-%m-%Y') as expirydate,e_documents.remarks,e_documents.createdby
                         FROM e_documents,in_lookup where e_documents.documenttype=in_lookup.lookcode and e_documents.staffid='".$PARENTID."' and entitytype='".$entitytype."' $addsql order by e_documents.id LIMIT $start1, $limit1";
	             ...
	             if($entitytype=='PO Documents'){
	              if(stripos(json_encode($_SESSION['role']),'PURCHASE MANAGER') == true){
	                    $entrydatatable.= "<td style='background-color:$colorbg;color:$colorfc;border:1px #ccc solid;' align='center'><a href='javascript:updateChildrecord(\"".$loginResultArrayChild['id']."\");'><img src='ico/edit.png' title='Update' width='16' height='16'></a></td>
	                      ...
	                      $entrydatatable.= "<td style='background-color:$colorbg;color:$colorfc;border:1px #ccc solid;' align='center'><a href='javascript:updateChildrecord(\"".$loginResultArrayChild['id']."\");'><img src='ico/edit.png' title='Update' width='16' height='16'></a></td>
	                                <td style='background-color:$colorbg;color:$colorfc;border:1px #ccc solid;' align='center'></td>";

	              }
	             }
	             else{
	             	/*if(($entitytype=="Profitcenter Documents" && stripos(json_encode($_SESSION['role']),'OPERATIONS MANAGER') != true && strtoupper($contractstatus)!='INACTIVE') || ($_REQUEST['entitytype']=="Ticket Documents" && stripos(json_encode($_SESSION['role']),'SITE INCHARGE') == true) || ($_REQUEST['entitytype']=="Supplier Documents" && stripos(json_encode($_SESSION['role']),'SERVICE COORDINATOR') == true) || ($_REQUEST['entitytype']=="Purchase Documents" && stripos(json_encode($_SESSION['role']),'SALES COORDINATOR') == true)          */
	             	
	             	if(($entitytype=="Profitcenter Documents" && strtoupper($contractstatus)!='INACTIVE' && $loginResultArrayChild['createdby'] == $_SESSION['SESSuserID']) || 
	             	($entitytype=="Ticket Documents" && stripos(json_encode($_SESSION['role']),'SITE INCHARGE') == true && $loginResultArrayChild['createdby'] == $_SESSION['SESSuserID']) || 
	             	($entitytype=="Supplier Documents" && stripos(json_encode($_SESSION['role']),'SERVICE COORDINATOR') == true) || 
	             	($entitytype=="Purchase Documents" && $loginResultArrayChild['createdby'] == $_SESSION['SESSuserID'])                                                          	){
	             $entrydatatable.= "<td style='background-color:$colorbg;color:$colorfc;border:1px #ccc solid;' align='center'><a href='javascript:updateChildrecord(\"".$loginResultArrayChild['id']."\");'><img src='ico/edit.png' title='Update' width='16' height='16'></a></td>
	                                <td style='background-color:$colorbg;color:$colorfc;border:1px #ccc solid;' align='center'><a href='javascript:deleteChildrecord(\"".$loginResultArrayChild['id']."\");'><img src='ico/remove.png' title='Remove' width='16' height='16'></a></td>";
				}
				else{
					$entrydatatable.= "<td style='background-color:$colorbg;color:$colorfc;border:1px #ccc solid;' align='center' colspan=2><img src='ico/posted.png' title='Update' width='16' height='16'></td>";
				}
	             
	             
	             }
	             //$entrydatatable.= "<td style='background-color:$colorbg;color:$colorfc'>" . $loginResultArrayChild['entitytype'] . "</td>";
...
function GetStatus($entitytype){
		global $con;
         ...
         $SEL =  "select lookcode,lookname from in_lookup_head where looktype='STATUS' and lookname<>'YY' order by id";
         $RES = mysqli_query($con,$SEL);
         while ($ARR = mysqli_fetch_array($RES)) {
                $SEL = "";
...
function GetDocument($documenttype,$entitytype){
		global $con;
         ...
         /*if($entitytype=="Supplier Documents" || $entitytype=="Purchase Documents" || $entitytype=="COMPANY_DOCUMENTS")
         $SEL =  "select lookcode,lookname from in_lookup where looktype='Supplier Documents' and lookname<>'XX' order by id";*/
         $RES = mysqli_query($con,$SEL);
         while ($ARR = mysqli_fetch_array($RES)) {
                $SEL = "";
...
function getattachments($id,$entitytype,$createdby){
	global $con;
     $SEL =  "select * from e_attachments where docid='".$id."' and doctype='$entitytype'";
     $RES = mysqli_query($con,$SEL);
     $childslno=1;
