...
//exit;
    ...
    for($i=0 ;$i < count($Htemp); $i++){
    	    if($formname=="menulist.php"){
                ...
                // deleting menus under userpriviledges
                $deletesql_2 = "delete from tbl_menusetup where parentid='".$Htemp[$i]."' or menucode='".$Htemp[$i]."'";
                $result   = mysqli_query($con,$deletesql_2)  or die(mysqli_error()."<br>".$deletesql_2);
                
                   
                UserLog($_SESSION['CurrentObjectName']->TableName,$Htemp[$i],$deletesql_child,"DELETE");
                UserLog("tbl_menu",$Htemp[$i],$deletesql,"DELETE");
                UserLog("tbl_menusetup",$Htemp[$i],$deletesql_2,"DELETE");
