...
if($_REQUEST['ID'] != "0") {
             ...
             $SQL = " Select * from tbl_states where id='".$_REQUEST['ID']."'";
             $SQLRes =  mysqli_query($con,$SQL) or die(mysqli_error()."<br>".$SQL);
              if(mysqli_num_rows($SQLRes)>=1){
                while($loginResultArray   = mysqli_fetch_array($SQLRes)){
                   ...
                   $countryid = $loginResultArray['countryid'];
                   $emirate = $loginResultArray['emirate'];
                  }
              }
           }else{
             $mode="";
             $saveid = GetLastSqeID("tbl_states");
             $countryid = $emirate = "";
}
if(isset($_REQUEST['dr'])=='view'){
   $edit="none";
   $view="inline";
   $title="Viewing Emirate : $emirate";
}else if(isset($_REQUEST['dr'])=='edit'){
      $edit="inline";
      $view="none";
      $title="Editing Emirate : $emirate";
}else{
      $edit="inline";
      $view="none";
...
function editingrecord(action)
{
       ...
       var cmb_A_countryid=document.getElementById('cmb_A_countryid');
       if(cmb_A_countryid){
          if ((cmb_A_countryid.value==null)||(cmb_A_countryid.value=="")){
               alertify.alert("Select Country", function () {
               cmb_A_countryid.focus();
       ...
       var txt_A_emirate=document.getElementById('txt_A_emirate');
       if(txt_A_emirate){
          if ((txt_A_emirate.value==null)||(txt_A_emirate.value=="")){
               alertify.alert("Enter Emirate Name", function () {
               txt_A_emirate.focus();
       ...
       insertfunction(parameter,action)
                   ...
                   function stateChangedsave()
                   {
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                 ...
                                 window.location.href='editemiratelist.php?dr=edit&ID='+document.getElementById('saveid').value;
                                });
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='editemiratelist.php?dr=edit&ID='+document.getElementById('mode').value;
                    ...
                    function stateChangedsavenew()
                   {
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                 alertify.alert("Record Saved", function () {
                                 window.location.href='editemiratelist.php?dr=add&ID=0';
                                });
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='editemiratelist.php?dr=add&ID=0';
                    ...
                    function stateChangedsaveclose()
                   {
                         ...
                         if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
                         {
                               ...
                               if(s1.toString() == s2.toString()){
                                ...
                                window.location.href='emiratelist.php?ID=0';
                               }else if(s1.toString() == s3.toString()){
                                alertify.alert("Record Updated", function () {
                                window.location.href='emiratelist.php';
...
<body class="hold-transition sidebar-mini">
         ...
         <section class="content-header">
                 ...
                 <a class="pull-left" href="emiratelist.php?pr=<?php echo $_SESSION['pr']; ?>&frmPage_rowcount=<?php echo $_SESSION['frmPage_rowcount']; ?>&txtsearch=<?php echo $_SESSION['txtsearch']; ?>&frmPage_startrow=<?php echo $_SESSION['frmPage_startrow'];?>" data-toggle="tooltip" data-placement="right" title="Back to Company Setup"><i class='fa fa-backward'></i></a>
                 <h2 class="title">&nbsp;&nbsp;<?php echo $title; ?></h2>

                 <!--<ol class='breadcrumb'>
                  <li><a href="#"><a href="blank.php" >Admin Setup</a></li>
                  <li><a href="#"><a href="emiratelist.php?ps=1">emiratelist</a></li>
                ...
                <section class="content" id='content-content-id' style='padding-right:5px;padding-left:5px;margin-top:-10px;' >
                  ...
                  <div class="nav-tabs-custom" id="nav-tabs-custom-id">
                        <ul class="nav nav-tabs">
                           ...
                           <li class="active"><a href="#personal" data-toggle="tab"><i class="fa fa-desktop" aria-hidden="true"></i>&nbsp; Emirate</a></li>
                       </ul>
...
<?php
        ...
        $entrydata = "<form name='frmEdit' method='post' id='frmEdit' enctype='multipart/form-data'>
                                 ...
                                 <table class='table table-bordered table-condensed table-fixed  table-responsive' style='table-layout:fixed'>
                                                            <tr>
                                                              <td class='dvtCellLabel' style='border: 1px solid #ccc;'>Emirate Code:</td>
                                                              ...
                                                              <td style='border: 1px solid #ccc;'>".GetCountry($countryid)."</td>
                                                              <td class='dvtCellLabel' style='border: 1px solid #ccc;'>Emirate:$mandatory</td>
                                                              <td style='border: 1px solid #ccc;'><input type='text' class='form-control txt inputs'  name='txt_A_emirate' id='txt_A_emirate'  value='$emirate' ></td>
                       ...
                       if($update == "true" || ($insert=="true" && $_REQUEST['ID']==0))
                        ...
                        $entrydata.="  <button class='btn btn-danger inputs' style='margin-top:-5px;' name='btndanger' type='button'  onclick ='javascript:closeediting(\"emiratelist.php?ps=1&pr=".$_SESSION['pr']."\");'>Close &nbsp;<i class='fa fa-close' aria-hidden='true'></i></button>
                                        </div>";
                        $entrydata.= "</form>  ";
echo  $entrydata;
?>
...
function GetCountry($countryid){
	global $con;
	$CMB = " <select name='cmb_A_countryid'  id='cmb_A_countryid' class='form-control select2'>";
