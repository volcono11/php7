...
<section class="content" id='content-wrapper-id' style='padding-right:5px;padding-left:5px;' >
...
$list_sql = "SELECT * FROM tbl_menu where 1 $addsql order by slno";//parentid='".$PARENTID."' 
...
$result = mysqli_query($con,$sql) or die(mysqli_error());
        $entrydatatable = "
        <table class='table table-bordered table-condensed table-fixed table-striped table-responsive' style='border:1px #2F3C43;'>";
...
function GetSubMenus($menu_name){
	global $con;
         ...
         $SEL =  "select menuname,url,iconname from tbl_objectmaster where showinmenu ='Yes' and objecttype='4001' order by menuname";
         $RES = mysqli_query($con,$SEL);
         while ($ARR = mysqli_fetch_array($RES)) {
                $SEL = "";
                if(strtoupper($menu_name) == strtoupper($ARR['menuname'])){ $SEL =  "SELECTED";}
                $CMB .= "<option value='".$ARR['menuname'].":".$ARR['url'].":".$ARR['iconname']."' $SEL >".$ARR['menuname']."</option>";
         }
         $CMB .= "</select>";
         return $CMB;
}
