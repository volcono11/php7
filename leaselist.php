...
<?php
		...
		$id_field = 'tbl_lease.id';
		$fieldNames = array('','Lease Code','Tenant Name','Property','Unit','Lease Amount');
		$fieldSizes = array('',10,20,20,20,20);
		$fieldAlign = array('','left','left','left','left','left');
		$fields = array('tbl_lease.id','B.tenantname','C.propertyname','A.unitname','tbl_lease.leaseamt');
		$table = 'tbl_lease left join tbl_propertydetails as A on tbl_lease.property=A.propertyid
        left join tbl_tenant as B on tbl_lease.tenant=B.id left join tbl_property as
        C on tbl_lease.property=C.id';
		$formlistname = "leaselist.php";
		$formeditlist= "editleaselist.php";
        ...
        $clause = " (tbl_lease.id LIKE '%$findBysearch%' or A.unitname LIKE '%$findBysearch%' or B.tenantname LIKE '%$findBysearch%'  or C.propertyname LIKE '%$findBysearch%' or leaseamt LIKE '%$findBysearch%')";//
        ...
        $grid->editFields = array("id,property");

        $grid->editRecord = $update;

        $grid->TableName = "tbl_lease";

        $grid->formName = "leaselist.php";
